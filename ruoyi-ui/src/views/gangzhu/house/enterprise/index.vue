<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['gangzhu:enterprise:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['gangzhu:enterprise:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['gangzhu:enterprise:remove']"
        >删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="batchList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="期限" align="center" min-width="200">
        <template slot-scope="scope">
          <span v-if="scope.row.checkInDate && scope.row.checkOutDate">
            {{ parseTime(scope.row.checkInDate, '{y}-{m}-{d}') }} ~ {{ parseTime(scope.row.checkOutDate, '{y}-{m}-{d}') }}
          </span>
    ...
    <el-dialog title="企业批次详情" :visible.sync="viewOpen" width="1100px" append-to-body>
      ...
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="入驻日期">
          {{ parseTime(viewData.checkInDate || viewData.selectionStartDate, '{y}-{m}-{d}') }}
        </el-descriptions-item>
        <el-descriptions-item label="截止日期">
          {{ parseTime(viewData.checkOutDate || viewData.selectionEndDate, '{y}-{m}-{d}') }}
        </el-descriptions-item>
        ...
        <el-descriptions-item label="批次状态">
          <el-tag v-if="viewData.batchStatus === '0'" type="warning">进行中</el-tag>
          <el-tag v-else-if="viewData.batchStatus === '1'" type="success">已完成</el-tag>
          <el-tag v-else-if="viewData.batchStatus === '2'" type="info">已作废</el-tag>
          <span v-else>-</span>
        </el-descriptions-item>
        ...
        <el-descriptions-item label="合同文件" :span="2">
          <el-link v-if="viewData.contractFile" :href="getContractUrl(viewData.contractFile)" target="_blank" type="primary" icon="el-icon-document">
            下载合同
          </el-link>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        checkInDate: [
          { required: true, message: "入驻日期不能��空", trigger: "change" }
        ],
        checkOutDate: [
