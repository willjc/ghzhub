<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="projectList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="640" fixed="right">
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="text"
    ...
    <el-dialog :title="upload.title" :visible.sync="upload.open" width="500px" append-to-body>
      <el-alert
        ...
        <div style="font-size: 12px; line-height: 1.8;">
          1. 模板自带一行"市场租赁"示例数据，请在填写前<strong>删除示例行</strong>（若保留，导入时自动跳过）<br/>
          2. 项目名称必填，按项目名称判重（已存在时提示）<br/>
          3. 项目类型填写：1人才公寓 / 2保租房 / 3市场租赁<br/>
          4. 状态填写：0正常 / 1停用<br/>
          5. 模板可下载后填写，格式勿修改
        </div>
      </el-alert>
