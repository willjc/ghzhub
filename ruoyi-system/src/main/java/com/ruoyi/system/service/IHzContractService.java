...
public interface IHzContractService {
     ...
     * 不调用微信退款 API，由调用方在事务外发起退款，再保存两笔退款结果。
     *
     * @param contractId   合同ID
    ...
    /** 保存自动退款中押金、租金各自结果（事务内执行）。 */
    void markCheckoutRecordRefundResult(Long applyId, boolean depositOk, boolean rentOk, String paymentRemark);
}
