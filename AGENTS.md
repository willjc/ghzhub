...
- 租赁设施保留数据库业务编码和 `RENTAL` 隔离，生成合同时由代码映射到 e签宝实际 `componentId`；后台只能选择当前项目类型允许的设施。
- 三类项目普通新签由服务端确定起租日为当天加3天，租期12个月、月付；续租衔接原合同到期日，批次配租沿用批次日期。
- 保租房、市场租赁新签押金固定1000元，续租沿用原合同押金；内部合同模板的旧付款周期不再决定新合同账单。
