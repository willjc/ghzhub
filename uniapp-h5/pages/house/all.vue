		...
		methods: {
			...
			async loadProjectList() {
				...
				try {
					...
					// 不传项目类型，查询所有类型（人才公寓、保租房、市场租赁）
					const response = await getProjectListByType(null, params)
					...
					console.log('加载所有房源项目列表成功:', this.listingData)
					...
					console.error('加载房源项目列表失败:', error)
					uni.showToast({
						title: '加载项目列表失败',
						icon: 'none'
					})
